...
def macro_NPMod(POStag):
    """
    defines if a token is a macro_NPMod or not
...
def macro_endings(POStag):
    """
    defines if a token is a possible ending in a complex name or not
    :param POStag: list of token, lemma, annotation.with.dots
    :return: true if it is a possible ending, false otherwise
    """
    return re.searcg('CN|OCCUP|GEO|FIRM|TITLE', POStag)
...
def rules(sent, j, window, curr_POStag, first_right_token, first_right_lemma, first_right_annot, outfile):
    if curr_POStag.endswith('NOM'):      
        ...
        if re.search('FN|DET_NM', curr_POStag) or (macro_NPMod(curr_POStag) and 'PL' in curr_POStag) or \
            ...
            elif ('TULN' == curr_POStag and macro_ending(window[0][2])):
            	# before possible endings in a complex name, if the current word is a proper name:
            	curr_POStag = curr_POStag.replace('NOM', semmi')
